...
export default function TypeTable({
  ...
  return (
    <table className="w-full text-[11px]">
      <caption className="panel-title mb-2 text-left">{t('journal.byType')}</caption>
      <thead>
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.type} className="border-t border-white/[0.05]">
            ...
            <td className="py-1.5 text-right font-mono tabular-nums text-slate-200">{r.count}</td>
            <td className="py-1.5 text-right font-mono tabular-nums text-slate-500">
              {formatPercent(r.share)}
            </td>
            <td className="py-1.5 text-right font-mono tabular-nums text-slate-500">{r.days}</td>
          </tr>
        ))}
      </tbody>
    </table>
