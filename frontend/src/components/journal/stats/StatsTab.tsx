...
import AlertDurations from './AlertDurations'
 ...
 * that same slice. */
export default function StatsTab({ onOpenDay }: { onOpenDay: (date: string) => void }) {
  ...
  if (phase === 'error') {
    return (
      <Section delay={2}>
        <div className="py-8 text-center text-xs text-slate-500">{t('journal.loadError')}</div>
      </Section>
    )
  }
  if (!stats) {
  ...
  return (
    ...
    <div className={phase === 'loading' ? 'opacity-50 transition-opacity' : 'transition-opacity'}>
      <Section delay={2}>
        <div className="mb-4 flex items-center justify-between gap-3">
          <PeriodSwitch value={period} onChange={setPeriod} disabled={phase === 'loading'} />
      ...
      {!hasTargets ? (
        ...
        <>
          <Section delay={3}>
            <HourOfDay hours={stats.hours} alertDaysObserved={stats.alert_days_observed} />
            {stats.alert_from_date && stats.alert_from_date > stats.from_date && (
              <p className="mt-3 border-t border-white/[0.06] pt-3 text-[11px] leading-relaxed text-slate-500">
                {t('journal.stats.alertWindowNote', { from: stats.alert_from_date })}
              </p>
            )}
          </Section>
          ...
          <Section delay={4}>
            ...
            <div className="mt-5 border-t border-white/[0.06] pt-4">
              <TypeTable typeTotals={stats.type_totals} typeDays={stats.type_days} />
            </div>
          </Section>

          <Section delay={5}>
            <AlertDurations
              buckets={stats.alert_durations}
              medianSeconds={stats.median_alert_seconds}
              meanSeconds={stats.mean_alert_seconds}
              longestSeconds={stats.totals.longest_alert_seconds}
            />
          </Section>

          <Section delay={6}>
            <DistrictBars districts={stats.districts} districtName={districtName} />
          </Section>

          <Section delay={7}>
            <HeaviestDays days={stats.days} locale={locale} onOpenDay={onOpenDay} />
          </Section>
        </>
      )}
    </div>
