...
export default function CitywidePulse() {
  ...
  return (
    <>
      {boundaries.map((b) => (
        // Key on colour so a type upgrade (missile -> ballistic) remounts the
        // path with the new stroke instead of silently keeping the old one.
        <GeoJSON
          key={`citypulse-${b.id}-${color}`}
          data={b.geojson}
          interactive={false}
          style={{ color, weight: 2, opacity: 0.55, fillColor: color, fillOpacity: 0.05,
